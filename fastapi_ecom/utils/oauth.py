...
async def current_user(token: str = Depends(oidc)) -> OIDCUser:
    """
    Extract and validate the current authenticated user from OIDC token.

    This function processes the Bearer token from the request, validates it with
    ...
    :return: The authenticated OIDC user object.

    :raises HTTPException: If the token is missing, malformed, or invalid.
    """
    if not token:
