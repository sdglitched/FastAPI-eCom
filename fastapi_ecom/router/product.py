...
from fastapi_ecom.database.pydantic_schemas.product import ProductCreate, ProductUpdate, ProductInternal, ProductResult, ProductManyResult
from fastapi_ecom.utils.crud.product import add_product, get_all_products, get_product_by_uuid, delete_product, modify_product
...
@router.post("", response_model=ProductResult, status_code=201, tags=["product"])
...
@router.get("/{uuid}", response_model=ProductResult, tags=["product"])
...
@router.delete("/delete/{uuid}", response_model=ProductResult, tags=["product"])
...
@router.put("/update/{uuid}", response_model=ProductResult, tags=["product"])
async def updt_product(uuid: str, product: ProductUpdate, db: AsyncSession = Depends(get_db), business_auth: BusinessInternal = Depends(verify_business_cred)):
    product_to_update = await modify_product(db=db, product=product, uuid=uuid)
    return {
        "action": "put",
